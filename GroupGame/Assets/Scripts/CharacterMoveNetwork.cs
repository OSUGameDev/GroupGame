...
public class CharacterMoveNetwork : NetworkBehaviour
{
    ...
    void Start()
    {
        //check to see f this player is controlled by this client
        if (hasAuthority)
        {
            //set the camera to follow it if it is
            GameObject.FindGameObjectWithTag("MainCamera").GetComponent<FollowPlayer>().target = this.transform;
        }
    }
    void Update()
    {
        //check to see if this player is controlled by this client
        if (!hasAuthority)
            return;
